...
def create_app(test_config=None):
    ...
    @app.route('/notes/<notes_id>/edit', methods=('GET', 'POST', 'PATCH')) #takes in note is
    @require_login
    def note_update(note_id):
        note = Note.query.filter_by(user_id=g.user.id, id=note_id).first_or_404() #query to find note by user & note id
    ...
    def note_delete(note_id):
        note = Note.query.filter_by(user_id=g.user.id, id=note_id).first_or_404() #search fot note
